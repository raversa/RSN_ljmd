...
int main(int argc, char **argv)
{
    ...
    handle = dlopen("../lib-serial/velverlet2.so", RTLD_LAZY);

    if (handle)
    {
        mdsys_t sys = get_me_three_atoms();
        func_to_be_tested = (void (*)(mdsys_t *sys)) dlsym(handle, "velverlet2");
        ...
        {   
            if (sys.vx[i]-expected_x[i] > 1e-15)
                error = 1;
            if (sys.vy[i]-expected_y[i] > 1e-15)
                error = 1;
            if (sys.vz[i]-expected_z[i] > 1e-15)
                error = 1;
        }
        if (error == 0)
            printf("test passed\n");
